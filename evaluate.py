...
from utils import loop_version_from_tag_table_to_triplets


def evaluate_model(model, test_dataset, test_dataloader, id2senti, device='cuda', version='3D', weight=None,
                   saved_file=None):
    model.eval()
    total_loss = 0.0
    total_step = 0
    ...
    if saved_file is not None:
        with open(saved_file, 'w', encoding='utf-8') as f:
            combined = [
                dict(token=token, pred=pred, gold=gold, pred_aspect=pred_aspect, pred_opinion=pred_opinion) for
                token, pred, gold, pred_aspect, pred_opinion in
                zip(saved_token, saved_preds, saved_golds, saved_aspects, saved_opinions)
            ]
            json.dump(combined, f)

    loss = total_loss / total_step
    evaluate_dict = evaluate_predictions(preds=saved_preds, goldens=saved_golds, preds_aspect=saved_aspects,
                                         preds_opinion=saved_opinions)
    model.train()
...
def print_evaluate_dict(evaluate_dict):
    ...
    for idx, m in enumerate(evaluate_dict):
        ...
        evaluate_result += print_dict(m, select_k=select_k)
